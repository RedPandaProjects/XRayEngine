...
void draw_planner						(const planner_type &brain, LPCSTR start_indent, LPCSTR indent, LPCSTR planner_id)
{
	...
	for ( ; I != E; ++I) {
		const auto J = std::lower_bound(brain.current_state().conditions().begin(),brain.current_state().conditions().end(),planner_type::CWorldProperty((*I).first,false));
		char				temp = '?';
	...
	for ( ; I != E; ++I) {
		const auto J = std::lower_bound(brain.target_state().conditions().begin(),brain.target_state().conditions().end(),planner_type::CWorldProperty((*I).first,false));
		char				temp = '?';
...
static void fill_bones				(CAI_Stalker& self, Fmatrix const& transform, IKinematicsAnimated* kinematics_animated, LPCSTR animation_id, bool const local)
{
	...
	IBoneInstance& root_bone			= kinematics->LL_GetBoneInstance(root_bone_id);
...
static void	_BCL test_callback			(IBoneInstance *B)
{
	VERIFY								(B);
