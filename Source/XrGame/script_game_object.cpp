...
Fvector	CScriptGameObject::bone_position	(LPCSTR bone_name) const
{
	...
	if (xr_strlen(bone_name))
		bone_id			= CastToIKinematics(object().Visual())->LL_BoneID(bone_name);
	else
		bone_id			= CastToIKinematics(object().Visual())->LL_GetBoneRoot();

	Fmatrix				matrix;
	matrix.mul_43		(object().XFORM(),CastToIKinematics(object().Visual())->LL_GetBoneInstance(bone_id).GetTransform());
	return				(matrix.c);
}
