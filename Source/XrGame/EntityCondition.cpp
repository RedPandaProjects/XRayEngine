...
{
	u64 _cur_time = (GameID() == eGameIDSingle) ? Level().GetGameTime() : Device->dwTimeGlobal;
...
//	case ALife::eHitTypePhysicStrike:
	...
	if (bDebug && !is_special_hit_2_self ) 
	{
		Msg("%s hitted in %s with %f[%f]", m_object->Name(), 
			CastToIKinematics(m_object->Visual())->LL_BoneName_dbg(pHDS->boneID), m_fHealthLost*100.0f, hit_power_org);
	}
