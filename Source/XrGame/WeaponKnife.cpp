...
{
	/*VERIFY(victim);
	IKinematics*	tmp_kinem	= CastToIKinematics(victim->Visual());
	...
	Fmatrix	& tmp_xform			= victim->XFORM();
	IBoneInstance &bi			= tmp_kinem->LL_GetBoneInstance();
...
u32 CWeaponKnife::get_entity_bones_count(CEntityAlive const * entity)
{
	...
	if (!entity)
		return 0;
	IKinematics*	tmp_kinem	= CastToIKinematics(entity->Visual());
	if (!tmp_kinem)
		return 0;
