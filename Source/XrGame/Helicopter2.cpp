...
//lighting
	if(m_light_render->get_active()){
		...
		if (m_lanim)
		{
			...
			m_light_render->set_intensity(m_light_brightness);
		}
...
void CHelicopter::DieHelicopter()
{
	...
	m_brokenSound.Create			(pSettings->r_string(*cNameSect(), "broken_snd"),SOUND_TYPE_FROM_SOURCE);
	m_brokenSound.Play		(0,XFORM().c,true);
