...
//		Msg						("%6d : Object [%d][%s][%s] is spawned DEAD",Device->dwTimeGlobal,ID(),*cName(),*cNameSect());
	...
	// Eyes
	eye_bone					= CastToIKinematics(Visual())->LL_BoneID(pSettings->r_string(cNameSect(),"bone_head"));
...
void CCustomMonster::ListenSound(CObject* InSourceActor, s32 SoundFlags,const Fvector& InPosition, float InnPower, CSound_UserDataPtr UserData)
{
	if (getDestroy())
		return;
	memory().sound().ListenSound(InSourceActor,SoundFlags,InPosition,InnPower,UserData);
}
