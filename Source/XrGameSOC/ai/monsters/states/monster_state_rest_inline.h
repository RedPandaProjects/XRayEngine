	...
	if (captured_by_smart_terrain) this->select_state(eStateSmartTerrainTask);
		...
		if (move_to_restrictor) this->select_state(eStateCustomMoveToRestrictor);
			...
			if (move_to_home_point) this->select_state(eStateRest_MoveToHomePoint);
				...
				if (!use_squad) {
					if (this->time_idle_selected + TIME_IDLE > time()) 			this->select_state	(eStateRest_Idle);
					else if (this->time_idle_selected + TIME_IDLE  + TIME_IDLE/2 > time()) 	this->select_state	(eStateRest_WalkGraphPoint);
					else {
						this->time_idle_selected	= time();
