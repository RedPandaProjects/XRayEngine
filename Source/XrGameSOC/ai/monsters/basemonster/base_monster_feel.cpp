...
void CBaseMonster::ListenSound				(CObject* who, s32 eType,const Fvector& Position, float power, CSound_UserDataPtr user_data)
{
	if (!g_Alive())		return;
...
void CBaseMonster::HitEntity(const CEntity *pEntity, float fDamage, float impulse, Fvector &dir)
{
	...
	if (EnemyMan.get_enemy() == pEntity) {
		...
		HS.boneID			= (CastToIKinematics(pEntityNC->Visual())->LL_GetBoneRoot());//		l_P.w_s16	(CastToIKinematics(pEntityNC->Visual())->LL_GetBoneRoot());
...
void CBaseMonster::HitSignal(float amount, Fvector& vLocalDir, CObject* who, s16 element)
{
	...
	ListenSound(who,SOUND_TYPE_WEAPON_SHOOTING,who->Position(),1.f,nullptr);
...
void CBaseMonster::Hit_Wound(CObject *object, float value, const Fvector &dir, float impulse) 
{
	...
	HS.boneID			= (CastToIKinematics(object->Visual())->LL_GetBoneRoot());	//	P.w_s16		(CastToIKinematics(object->Visual())->LL_GetBoneRoot());
	HS.p_in_bone_space	= (Fvector().set(0.f,0.f,0.f));										//	P.w_vec3	(Fvector().set(0.f,0.f,0.f));
