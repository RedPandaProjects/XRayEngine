...
bool CWeaponMagazinedWGrenade::CanAttach(PIItem pIItem)
{
	...
	if(pGrenadeLauncher &&
	   CSE_ALifeItemWeapon::EWeaponAddonStatus::eAddonAttachable == m_eGrenadeLauncherStatus &&
...
{
	if(CSE_ALifeItemWeapon::EWeaponAddonStatus::eAddonAttachable == m_eGrenadeLauncherStatus &&
	   0 != (m_flagsAddOnState&CSE_ALifeItemWeapon::eWeaponAddonGrenadeLauncher) &&
...
void CWeaponMagazinedWGrenade::UpdateGrenadeVisibility(bool visibility)
{
	...
	IKinematics* pHudVisual						= CastToIKinematics(m_pHUD->Visual());
	VERIFY										(pHudVisual);
