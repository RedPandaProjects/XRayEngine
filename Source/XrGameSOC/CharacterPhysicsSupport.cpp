...
void CCharacterPhysicsSupport::in_NetSpawn(CSE_Abstract* e)
{
	...
	IKinematicsAnimated*ka= CastToIKinematicsAnimated(m_EntityAlife.Visual());
	...
	if(Type() == etStalker)
	{
		m_hit_animations.SetupHitMotions(*CastToIKinematicsAnimated(m_EntityAlife.Visual()));
	}
	anim_mov_state.init();
...
/*
		IKinematicsAnimated	*K = CastToIKinematicsAnimated(m_EntityAlife.Visual());
...
void CCharacterPhysicsSupport::CreateSkeleton(CPhysicsShell* &pShell)
{
	...
	pShell->preBuild_FromKinematics(CastToIKinematics(m_EntityAlife.Visual()));
	...
	disable_params.Load(CastToIKinematics(m_EntityAlife.Visual())->LL_UserData());
...
void CCharacterPhysicsSupport::CreateSkeleton()
{
	...
	m_pPhysicsShell->build_FromKinematics(CastToIKinematics(m_EntityAlife.Visual()));
	...
	CastToIKinematics(m_EntityAlife.Visual())->CalculateBones();
	//b_death_anim_on=false;
	m_flags.set(fl_death_anim_on,FALSE);
	m_eState=esDead;
}
