	...
	)
{
	...
	this->m_distance_xz			= this->graph->header().cell_size();
	this->m_sqr_distance_xz		= _sqr(this->graph->header().cell_size());
...
//		return					(_sqrt(square_size_y*(float)_sqr(y2 - y1) + m_sqr_distance_xz));
	return					(this->m_distance_xz);// + (y2 - y1)*size_y);
}
...
//		return					(_sqrt((float)(m_sqr_distance_xz*float(_sqr(x3 - x1) + _sqr(z3 - z1)) + square_size_y*(float)_sqr(y3 - y1))));
	return					(2* this->m_distance_xz*_dist_type(_abs(x3 - x1) + _abs(z3 - z1)));// + _abs(y3 - y1)*size_y);
//		int						x = _abs(x3 - x1);
//		int						z = _abs(z3 - z1);
