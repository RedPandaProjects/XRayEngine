...
void CCar::CreateSkeleton(CSE_Abstract	*po)
{
	...
	{
		K->PlayCycle		("idle");
		K->dcast_PKinematics()->CalculateBones	(TRUE);
	}

#pragma todo(" replace below by P_build_Shell or call inherited")
	m_pPhysicsShell		= P_create_Shell();
	m_pPhysicsShell->build_FromKinematics(CastToIKinematics(Visual()),&bone_map);
	...
	ApplySpawnIniToPhysicShell(CastToIKinematics(Visual())->LL_UserData(),m_pPhysicsShell,false);
}
