	...
	case GAME_PHASE_INPROGRESS:
		{
			...
			if (m_bSpectatorMode)
			{
				if (!m_pSM_CurViewEntity || m_pSM_CurViewEntity->CLS_ID != CLSID_OBJECT_ACTOR || m_dwSM_LastSwitchTime<Device->dwTimeGlobal)
...
bool	game_sv_Deathmatch::HasChampion()
{
	...
	m_server->ForEachClientDo(Lamda2);
