...
template<typename  typeVertex>
struct itterate_adjacents_params_dynamic_without_smoothgroups
{
	typedef	typeVertex											type_vertex;
	typedef	typename typeVertex::type_face						type_face;
	//typedef xvector<type_face*>								vecFace;
...
private:
	...
	u32				m_FaceCount;
	U8Vec			m_procesed;
	const float	    m_sm_cos;
public:
	itterate_adjacents_params_dynamic_without_smoothgroups(
...
private:
	...
	IC bool do_connect_faces(u32 start_face_idx, u32 test_face_idx, u16 start_common_edge_idx, u16 test_common_edge_idx, float sm_cos)
	{
		float	cosa = m_FaceNormals[start_face_idx].dotproduct(m_FaceNormals[test_face_idx]);
		return (cosa > sm_cos);
	}
...
public:
	...
	IC bool add_adjacents(u32 start_face_adj_idx, u32 test_face_adj_idx)
	{
		...
		if (has_same_edge(start_face, test_face, StartFace_common_edge_index, TestFace_common_edge_index))
		{
			if ((start_face_idx == test_face_idx) || do_connect_faces(test_face_idx, start_face_idx, StartFace_common_edge_index, TestFace_common_edge_index, m_sm_cos))
			{
				set_processed(test_face_adj_idx);
				add_adjacents(test_face_idx);
				return true;
