...
#include "dds.h"
#include <ddraw.h>
#include "..\..\BearBundle\External\Public\Nvtt\nvtt\nvtt.h"
						...
						STextureParams* fmt, u32 depth)
{
	...
	compOpt.setQuality(nvtt::Quality_Fastest); // HOT! Don't change to "high", unless u want to make compile nvtt to use CUDA
	compOpt.setQuantization(fmt->flags.is(STextureParams::flDitherColor) != 0, false, fmt->flags.is(STextureParams::flBinaryAlpha) != 0);
	...
	Msg("DXT: Compressing Image: 1 [Common Preparing]. Time from start %f ms", T.GetElapsed_sec() * 1000.f);

	if (fmt->flags.is(STextureParams::flGenerateMipMaps) && STextureParams::kMIPFilterAdvanced == fmt->mip_filter)
	{
		Msg("DXT: Compressing Image: 1 0 [Starting...]. Time from start %f ms", T.GetElapsed_sec() * 1000.f);
		...
		Msg("DXT: Compressing Image: 1 1 [Build32MipLevel]. Time from start %f ms", T.GetElapsed_sec() * 1000.f);
		...
		Msg("DXT: Compressing Image: 1 2 [Praparing rgba data]. Time from start %f ms", T.GetElapsed_sec() * 1000.f);
		...
		Msg("DXT: Compressing Image: 1 3 [NVTT Compression Started]. Time from start %f ms", T.GetElapsed_sec() * 1000.f);

		result = nvtt::Compressor().process(inOpt, compOpt, outOpt);

		Msg("DXT: Compressing Image: 1 4 [NVTT Compression Finished]. Time from start %f ms", T.GetElapsed_sec() * 1000.f);

		xr_free(pImagePixels);
	}
	else
	{
		Msg("DXT: Compressing Image: 2 1 [Starting...]. Time from start %f ms", T.GetElapsed_sec() * 1000.f);
		...
		// xrOxygen to LA: Fixed R and B channels substetution ;)
		for (u32 k = 0; k < w * h; k++, pixel += 4)
			pixels[k].set(pixel[0], pixel[1], pixel[2], pixel[3]);

		inOpt.setMipmapData(pixels, w, h);

		Msg("DXT: Compressing Image: 2 2 [NVTT Compression Started]. Time from start %f ms", T.GetElapsed_sec() * 1000.f);

		result = nvtt::Compressor().process(inOpt, compOpt, outOpt);

		Msg("DXT: Compressing Image: 2 3 [NVTT Compression Finished]. Time from start %f ms", T.GetElapsed_sec() * 1000.f);
	}
