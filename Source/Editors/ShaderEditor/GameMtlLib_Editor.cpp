...
//------------------------------------------------------------------------------
void  SGameMtlPair::OnFlagChange(PropValue* sender)
{
...
void  SGameMtlPair::OnParentClick(ButtonValue* V, bool& bModif, bool& bSafe)
{
    bModif = false;
    switch (V->btn_num)
    {
    case 0:
    {
        ...
        xr_string nm	= P?m_Owner->MtlPairToName(P->GetMtl0(),P->GetMtl1()):NONE_CAPTION;
        UIChooseForm::SelectItem(smCustom, 1, (nm == NONE_CAPTION) ? 0 : nm.c_str(), TOnChooseFillItems(this, &SGameMtlPair::FillChooseMtl));
        m_EditParent = true;
    }break;
	}
}

void  SGameMtlPair::OnCommandClick(ButtonValue* V, bool& bModif, bool& bSafe)
{
  bModif = false;
    switch (V->btn_num){
    case 0: {
        SGameMtlPair* P = m_Owner->GetMaterialPair(ID_parent);
        xr_string nm = P ? m_Owner->MtlPairToName(P->GetMtl0(), P->GetMtl1()) : NONE_CAPTION;
        UIChooseForm::SelectItem(smCustom, 128, 0, TOnChooseFillItems(this, &SGameMtlPair::FillChooseMtl));
        m_EditCommand = true;
    }
    break;
	}
...
void  SGameMtlPair::OnDrawUI()
{
    if (m_EditParent)
    {
        ...
        if (UIChooseForm::GetResult(change, result))
        {
            ...
            {
                int m0, m1;
                m_Owner->NameToMtlPair(result.c_str(), m0, m1);
