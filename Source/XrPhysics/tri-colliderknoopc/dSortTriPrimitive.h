...
)
{
	...
	for (I = B; I != E; ++I)
	{
		...
		if (debug_output().ph_dbg_draw_mask().test(phDBgDrawIntersectedTries))
			debug_output().DBG_DrawTri(Tris, V_array, D3DCOLOR_XRGB(0, 255, 0));
		...
		if (tri.dist < 0.f) {
			...
			if (debug_output().ph_dbg_draw_mask().test(phDBgDrawNegativeTries))
				debug_output().DBG_DrawTri(Tris, V_array, D3DCOLOR_XRGB(0, 0, 255));
#endif
			...
			if (debug_output().ph_dbg_draw_mask().test(phDBgDrawPositiveTries))
				debug_output().DBG_DrawTri(Tris, V_array, D3DCOLOR_XRGB(255, 0, 0));
#endif	
			if (ret > flags - 10)
				continue;
