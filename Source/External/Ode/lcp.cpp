...
struct dLCP {
  ...
  void transfer_i_to_N (int i)
    {
	  (void)i; nN++; }			// because we can assume C and N span 1:i-1
  void transfer_i_from_N_to_C (int i);
  void transfer_i_from_C_to_N (int i);
  int numC() { return nC; }
  int numN() { return nN; }
  ...
  void pC_plusequals_s_times_qC (dReal *p1, dReal s, dReal *q)
    { for (int i=0; i<nC; i++) p1[i] += s*q[i]; }
  void pN_plusequals_s_times_qN (dReal *p1, dReal s, dReal *q)
    { for (int i=0; i<nN; i++) p1[i+nC] += s*q[i+nC]; }
  void solve1 (dReal *a, int i, int dir=1, int only_transfer=0);
  void unpermute();
};
...
void dLCP::pN_equals_ANC_times_qC (dReal *p1, dReal *q)
{
  ...
  for (int i=0; i<nN; i++) p1[i+nC] = dDot (AROW(i+nC),q,nC);
}
...
void dLCP::pN_plusequals_ANi (dReal *p1, int i, int sign)
{
  ...
  if (sign > 0) {
    for (int i1=0; i1<nN; i1++) p1[i1+nC] += aptr[i1];
  }
  else {
    for (int i1=0; i1<nN; i1++) p1[i1+nC] -= aptr[i1];
  }
...
void dLCP::unpermute()
{
  ...
  for (j=0; j<n; j++) x[p[j]] = tmp1[j];
  memcpy (tmp1,w,n*sizeof(dReal));
  for (j=0; j<n; j++) w[p[j]] = tmp1[j];
}
