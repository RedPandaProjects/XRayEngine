...
{
	auto item = std::find_if(HW->FontRenders.begin(), HW->FontRenders.end(), [&](XRayFontRender* right)->bool {return right == this; });
	R_ASSERT(item != HW->FontRenders.end());
...
void XRayFontRender::OnRender(CGameFont& owner)
{
	...
	if (!(owner.uFlags & CGameFont::fsValid)) {
		...
		if (Blender.E[0][0].Textures[0])
		{
			auto size = Blender.E[0][0].Textures[0]->GetSize();
	...
	for (u32 i = 0; i < owner.strings.size(); ) {
		...
		if (vCount)
		{
			GRenderInterface.UpdateDescriptorHeap(Blender.E[0][0]);
			if (!Blender.E[0][0].Set(HW->Context, FVF::F_TL)) { return; }
