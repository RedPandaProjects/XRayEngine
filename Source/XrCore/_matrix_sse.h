...
public:
	...
	{
		R[0] = _mm_setr_ps(a1, a2, a3, a4);
		R[1] = _mm_setr_ps(b1, b2, b3, b4);
		R[2] = _mm_setr_ps(c1, c2, c3, c4);
		R[3] = _mm_setr_ps(d1, d2, d3, d4);
	}
	...
	{
		VERIFY(Row < 4);
		R[Row] = _mm_mul_ps(R[Row], _mm_setr_ps(1, 1, 1, 0));
		R[Row] = _mm_add_ps(R[Row], _mm_setr_ps(x, y, z, 0));
	}
	...
	{
		VERIFY(Row < 4);
		R[Row] = _mm_add_ps(R[Row], _mm_setr_ps(x, y, z, w));
	}
	...
	{
		R[0] = _mm_setr_ps(r.x, r.y, r.z, 0);
		R[1] = _mm_setr_ps(N.x, N.y, N.z, 0);
		R[2] = _mm_setr_ps(D.x, D.y, D.z, 0);
		R[3] = _mm_setr_ps(C.x, C.y, C.z, 1);
		return *this;
	}
	IC	SelfRef	identity(void)
	{
		R[0] = _mm_setr_ps(1, 0, 0, 0);
		R[1] = _mm_setr_ps(0, 1, 0, 0);
		R[2] = _mm_setr_ps(0, 0, 1, 0);
		R[3] = _mm_setr_ps(0, 0, 0, 1);
		return *this;
	}
	...
	IC	SelfRef	mul_43(const Self& A, const Self& B)
	{
		...
		__m128 RMask = _mm_setr_ps(1, 1, 1, 0);
	...
	IC	SelfRef	translate(const Tvector& Loc)		// setup translation matrix
	{
		...
		R[1] = _mm_setr_ps(0, 1, 0, 0);
		R[2] = _mm_setr_ps(0, 0, 1, 0);
		R[3] = _mm_setr_ps(Loc.x, Loc.y, Loc.z, 1);
		return *this;
	}
	...
	IC	SelfRef	rotateX(float Angle)				// rotation about X axis
	{
		...
		R[0] = _mm_setr_ps(1, 0, 0, 0);
		R[1] = _mm_setr_ps(0, cosa, sina, 0);
		R[2] = _mm_setr_ps(0, -sina, cosa, 0);
		R[3] = _mm_setr_ps(0, 0, 0, 1);
		return *this;
	}
	IC	SelfRef	rotateY(float Angle)				// rotation about Y axis
	{
		...
		R[0] = _mm_setr_ps(cosa, 0, -sina,0);
		R[1] = _mm_setr_ps(0, 1, 0, 0);
		R[2] = _mm_setr_ps(sina, 0, cosa, 0);
		R[3] = _mm_setr_ps(0, 0, 0, 1);
	...
	IC	SelfRef	rotateZ(float Angle)				// rotation about Z axis
	{
		...
		R[0] = _mm_setr_ps(cosa, sina, 0, 0);
		R[1] = _mm_setr_ps(-sina, cosa, 0, 0);
		R[2] = _mm_setr_ps(0, 0, 1, 0);
		R[3] = _mm_setr_ps(0, 0, 0, 1);
	...
	IC	SelfRef	rotation(const Tvector& vdir, const Tvector& vnorm) {
		...
		R[0] = _mm_setr_ps(vright.x, vright.y, vright.z, 0);
		R[1] = _mm_setr_ps(vnorm.x, vnorm.y, vnorm.z, 0);
		R[2] = _mm_setr_ps(vdir.x, vdir.y, vdir.z, 0);
		R[3] = _mm_setr_ps(0, 0, 0, 1);
		return *this;
	}
	...
	{
		R[0] = _mm_setr_ps(1, 0, 0, 0);
		R[1] = _mm_setr_ps(0, 0, 1, 0);
		R[2] = _mm_setr_ps(0, 1, 0, 0);
		R[3] = _mm_setr_ps(0, 0, 0, 1);
		return *this;
	}
	IC	SelfRef	mapYXZ() 
	{
		R[0] = _mm_setr_ps(0, 1, 0, 0);
		R[1] = _mm_setr_ps(1, 0, 0, 0);
		R[2] = _mm_setr_ps(0, 0, 1, 0);
		R[3] = _mm_setr_ps(0, 0, 0, 1);
		return *this; 
	}
	IC	SelfRef	mapYZX() 
	{
		R[0] = _mm_setr_ps(0, 1, 0, 0);
		R[1] = _mm_setr_ps(0, 0, 1, 0);
		R[2] = _mm_setr_ps(1, 0, 0, 0);
		R[3] = _mm_setr_ps(0, 0, 0, 1);
		return *this; }
	IC	SelfRef	mapZXY() 
	{
		R[0] = _mm_setr_ps(0, 0, 1, 0);
		R[1] = _mm_setr_ps(1, 0, 0, 0);
		R[2] = _mm_setr_ps(0, 1, 0, 0);
		R[3] = _mm_setr_ps(0, 0, 0, 1);
		return *this; 
	...
	{
		R[0] = _mm_mul_ps(R[0], _mm_setr_ps(0, 1, 1,1));
		R[0] = _mm_add_ps(R[0], _mm_setr_ps(-1, 0, 0, 0));
		return *this;
	}
	IC	SelfRef	mirrorX_add()
	{
		R[0] = _mm_mul_ps(R[0], _mm_setr_ps(-1, 0, 0, 0));
		return *this;
	}
	...
	{
		R[1] = _mm_mul_ps(R[1], _mm_setr_ps(1, 0, 1, 1));
		R[1] = _mm_add_ps(R[1], _mm_setr_ps(0, -1, 0, 0));
		return *this;
	}
	IC	SelfRef	mirrorY_add() 
	{
		R[1] = _mm_mul_ps(R[1], _mm_setr_ps(0, -1, 0, 0));
		return *this;
	}
	...
	{
		R[2] = _mm_mul_ps(R[2], _mm_setr_ps(1, 1, 0, 1));
		R[2] = _mm_add_ps(R[2], _mm_setr_ps(0, 0, -1, 0));
		return *this;
	}
	IC	SelfRef	mirrorZ_add()
	{
		R[2] = _mm_mul_ps(R[2], _mm_setr_ps(0, 0, -1, 0));
		return *this;
	}
	IC	SelfRef	mul(const Self& A, float v) 
	{
		__m128 V = _mm_setr_ps(v, v, v, v);
	...
	IC	SelfRef	build_projection_HAT(float HAT, float fAspect, float fNearPlane, float fFarPlane)
	{
		...
		R[0] = _mm_setr_ps(w, 0, 0, 0);
		R[1] = _mm_setr_ps(0, h, 0, 0);
		R[2] = _mm_setr_ps(0, 0, Q, 1.f);
		R[3] = _mm_setr_ps(0, 0, -Q*fNearPlane, 0);
		return *this;
	}
	IC	SelfRef	build_projection_ortho(float w, float h, float zn, float zf)
	{

		R[0] = _mm_setr_ps(float(2) / w, 0, 0, 0);
		R[1] = _mm_setr_ps(0, float(2) / h, 0, 0);
		R[2] = _mm_setr_ps(0, 0, float(1) / (zf - zn), 0);
		R[3] = _mm_setr_ps(0, 0, zn / (zn - zf), 1);
		return *this;
	}
	IC	SelfRef	build_camera(const Tvector& vFrom, const Tvector& vAt, const Tvector& vWorldUp)
	{
		...
		R[0] = _mm_setr_ps(vRight.x, vUp.x, vView.x, 0);
		R[1] = _mm_setr_ps(vRight.y, vUp.y, vView.y, 0);
		R[2] = _mm_setr_ps(vRight.z, vUp.z, vView.z, 0);
		R[3] = _mm_setr_ps(-vFrom.dotproduct(vRight), -vFrom.dotproduct(vUp), -vFrom.dotproduct(vView), 1);
		return *this;
	}
	...
	IC	void	transform_tiny(Tvector& dest, const Tvector& v)	const // preferred to use
	{
		...
		__m128 RegV = _mm_setr_ps(v.x, v.y, v.z, 0);
	...
	{
		__m128 RegV = _mm_setr_ps(v.x, v.y, 0, 0);
		__m128 RegResult = _mm_mul_ps(swizzle<0,0,0,0>(RegV), R[0]);
		RegResult = _mm_add_ps(RegResult, _mm_mul_ps(swizzle<1,1,1,1>(RegV), R[1]));
		RegResult = _mm_add_ps(RegResult, R[3]);
		float Result[4];
	...
	{
		__m128 RegV = _mm_setr_ps(v.x, v.y, v.z, v.w);
		__m128 RegResult = _mm_mul_ps(swizzle<0,0,0,0>(RegV), R[0]);
		RegResult = _mm_add_ps(RegResult, _mm_mul_ps(swizzle<1, 1, 1, 1>(RegV), R[1]));
		RegResult = _mm_add_ps(RegResult, _mm_mul_ps(swizzle<2, 2, 2, 2>(RegV), R[2]));
		float Result[4];
	...
	IC	SelfRef	setHPB(float h, float p, float b)
	{
		...
		R[0] = _mm_setr_ps(_cc - _sp * _ss, -_cp * _sb, _sp * _cs + _sc, 0);
		R[1] = _mm_setr_ps(_sp * _sc + _cs, _cp * _cb, _ss - _sp * _cc, 0);
		R[2] = _mm_setr_ps(-_cp * _sh, _sp, _cp * _ch, 0);
		R[3] = _mm_setr_ps(0, 0, 0, 1);
