...
bool can_capture( CPHCharacter   *a_character, CPhysicsShellHolder	*a_taget_object  )
{
	...
	IKinematics* p_kinematics = CastToIKinematics( a_character->PhysicsRefObject()->Visual( ) );
	VERIFY( p_kinematics );
...
bool can_capture( CPHCharacter   *a_character, CPhysicsShellHolder	*a_taget_object, u16 a_taget_element  ) 
{
	...
	IKinematics* K=	CastToIKinematics( a_taget_object->Visual( ) );
...

{
	...
	m_taget_element			=m_taget_object->m_pPhysicsShell->NearestToPoint( m_capture_bone->GetTransform().c, cb );
	if( ! m_taget_element )
		return;
	Init( );
}
...

{
	...
	IKinematics* K=	CastToIKinematics( m_taget_object->Visual( ) );
	VERIFY( K );
...
void CPHCapture::Init( )
{
	...
	IKinematics* p_kinematics = CastToIKinematics( m_character->PhysicsRefObject()->Visual( ) );
	VERIFY( p_kinematics );
	CInifile	* ini = p_kinematics->LL_UserData();
	VERIFY( ini );
	...
	capture_bone_position.set(m_capture_bone->GetTransform().c);
