...
CStateGroupAttackAbstract::CStateGroupAttack(_Object *obj) : inherited(obj)
{
	...
	this->add_state	(eStateAttack_MoveToHomePoint,	xr_new<CStateGroupAttackMoveToHomePoint<_Object> >	(obj));
...
{
	return ai().level_graph().valid_vertex_id( this->object->EnemyMan.get_enemy_vertex() );
}
...
TEMPLATE_SPECIALIZATION
void CStateGroupAttackAbstract::remove_links(CObject* Inobject)
{
	if ( m_enemy == Inobject )
	{
		m_enemy	= 0;
